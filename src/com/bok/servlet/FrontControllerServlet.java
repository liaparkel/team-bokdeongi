...
public class FrontControllerServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response)
        ...
        // 4-1) 관리자용 상세보기 AJAX 분기
        if ("soloAskDetailManager".equals(cmd) && "true".equals(request.getParameter("ajax"))) {
            String askNum = request.getParameter("askNum");
            SoloAskManagerVO vo = new SoloAskManagerService().soloAskDetail(askNum);
            if (vo == null) {
                response.setStatus(HttpServletResponse.SC_NOT_FOUND);
                writeJson(response, Collections.singletonMap("error", "문의글이 없습니다."));
                return;
        ...
        if ("getSprtCategory".equals(cmd) || "getSprtPerson".equals(cmd) ||
            "getSprtContent".equals(cmd) || "getBfSprtPerson".equals(cmd) ||
            "getBfSprtManagerPerson".equals(cmd) || "getCkCategory".equals(cmd) ||
